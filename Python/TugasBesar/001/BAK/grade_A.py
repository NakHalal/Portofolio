...
def add_grade_by_assignment(dict_submission, dict_grade):
    ...
    #jawaban anda di bawah ini
    TRD = input("Masukkan NIM yang ingin dinilai: ")
    DFE = dict_mhs["data"]
    for key in list(DFE.keys()):
        if key == TRD:
            print('Detil Assignment...')
            DD = dict(DFE[TRD])
            for k, v in dict_activity.items():
                zxc = dict(v)
                if k not in dict_grade.keys():
                    if zxc["Type"] == "assignment":
                        cxz = dict(dict_submission[k])
                        print(zxc)
                        print("ID          \t:", k)
                        print("Title       \t:", zxc["Title"])
                        print("Description \t:", zxc["Description"])
                        print()
                        print("Jawaban Mahasiswa: \n{}\n".format(cxz[TRD]))
                        try:
                            PLH = int(input("Masukkan nilai (0-100): "))
                        except Exception:
                            pass
                        else:
                            if 0 <= PLH <= 100:
                                #lpj = dict(dict_grade[k])
                                dict_grade[k] = {TRD : PLH} 
                                pass
    print(dict_grade)
    print("\n")
    try:
...
if __name__ == "__main__":
    ...
    add_grade_by_assignment(dict_submission, dict_grade)
